...
# benchmark matmul overtensor sizes, maybe only will be able to get flops for matmul
...
N = [32, 64, 128, 256, 512, 1024, 2048, 4096]; st = []; flops = []
for n in N:
  FLOPS = n*n*n*2
  ...
  tm = min([waffle_prog(a, b) for _ in range(10)])
  st.append(tm*1e6); flops.append(FLOPS*1e-9/tm)

# matmul time
plt.figure(figsize=(16, 6))
plt.plot(N, st, marker='.', color=np.random.rand(3,))
...
# matmul flops
plt.figure(figsize=(16, 6))
plt.plot(N, flops, marker='.', color=np.random.rand(3,))
plt.xlabel('tensor size')
plt.ylabel('GFLOPS')
