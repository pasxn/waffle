...
for input in input_times:
  ...
  torch_times.append((time.perf_counter() - st)*1e3/input)
  torch_times_sum.append((time.perf_counter() - st)*1e3)

for input in input_times:
  st = time.perf_counter()
  for i in range(input):
    waffle_output = waffle_linear(tensor(input_img).transpose()).data
  waffle_times.append((time.perf_counter() - st)*1e3/input)
  waffle_times_sum.append((time.perf_counter() - st)*1e3)
